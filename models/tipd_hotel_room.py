...
class HotelFloor(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if "name" in vals:
                existing_floor = self.env["tipd_hotel.floor"].search(
...
class HotelRoom(models.Model):
    ...
    room_categ_id = fields.Many2one(
        "tipd_hotel.room.type", "Room Category", required=True, ondelete="restrict"
    )
    room_amenities_ids = fields.Many2many(
        "tipd_hotel.room.amenities", string="Room Amenities", help="List of room amenities."
    )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if "name" in vals:
                existing_room = self.env["tipd_hotel.room"].search(
                    [("name", "ilike", vals["name"])], limit=1
                )
                if existing_room:
                    raise ValidationError(
            ...
            if "room_categ_id" in vals:
                room_categ = self.env["tipd_hotel.room.type"].browse(
    ...
    def write(self, vals):
        ...
        """
        if "room_categ_id" in vals:
            room_categ = self.env["tipd_hotel.room.type"].browse(vals.get("room_categ_id"))
            vals.update({"categ_id": room_categ.product_categ_id.id})
...
class HotelRoomType(models.Model):
    ...
    _name = "tipd_hotel.room.type"
    _description = "Room Type"
    _inherits = {'product.category': 'product_categ_id'}

    categ_id = fields.Many2one("tipd_hotel.room.type", "Category")
    child_ids = fields.One2many("tipd_hotel.room.type", "categ_id", "Room Child Categories")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if "name" in vals:
                existing_room_type = self.env["tipd_hotel.room.type"].search(
                    [("name", "ilike", vals["name"])], limit=1
                )
            ...
            if "categ_id" in vals:
                room_categ = self.env["tipd_hotel.room.type"].browse(vals.get("categ_id"))
...
class HotelRoomAmenitiesType(models.Model):
    ...
    _name = "tipd_hotel.room.amenities.type"
    _description = "amenities Type"
    _inherits = {'product.category': 'product_categ_id'}

    amenity_id = fields.Many2one("tipd_hotel.room.amenities.type", "Category")
    child_ids = fields.One2many(
        "tipd_hotel.room.amenities.type", "amenity_id", "Amenities Child Categories"
    )
    product_categ_id = fields.Many2one(
        "product.category",
        "Product Category",
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if "name" in vals:
                existing_amenities_type = self.env["tipd_hotel.room.amenities.type"].search(
                    [("name", "ilike", vals["name"])], limit=1
                )
            ...
            if "amenity_id" in vals:
                amenity_categ = self.env["tipd_hotel.room.amenities.type"].browse(
                    vals.get("amenity_id")
                )
...
class HotelRoomAmenities(models.Model):
    ...
    _name = "tipd_hotel.room.amenities"
    ...
    amenities_categ_id = fields.Many2one(
        "tipd_hotel.room.amenities.type",
        "Amenities Category",
        required=True,
        ondelete="restrict",
    )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if "name" in vals:
                existing_room_amenities = self.env["tipd_hotel.room.amenities"].search(
                    [("name", "ilike", vals["name"])], limit=1
                )
            ...
            if "amenities_categ_id" in vals:
                amenities_categ = self.env["tipd_hotel.room.amenities.type"].browse(
