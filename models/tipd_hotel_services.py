...
class HotelServices(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if "name" in vals:
                existing_services = self.env["tipd_hotel.services"].search(
        ...
        if "service_categ_id" in vals:
            service_categ = self.env["tipd_hotel.service.type"].browse(
    ...
    def write(self, vals):
        ...
        """
        if "service_categ_id" in vals:
            service_categ_id = self.env["tipd_hotel.service.type"].browse(
                vals.get("service_categ_id")
            )
...
class HotelServiceType(models.Model):
    ...
    _name = "tipd_hotel.service.type"
    _description = "Service Type"
    _inherits = {'product.category': 'product_categ_id'}

    service_id = fields.Many2one("tipd_hotel.service.type", "Service Category")
    child_ids = fields.One2many(
        "tipd_hotel.service.type", "service_id", "Service Child Categories"
    )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if "name" in vals:
                existing_service_type = self.env["tipd_hotel.service.type"].search(
                    [("name", "ilike", vals["name"])], limit=1
                )
