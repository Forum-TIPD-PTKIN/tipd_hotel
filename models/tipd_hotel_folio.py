...
class FolioRoomLine(models.Model):
    ...
    room_id = fields.Many2one("tipd_hotel.room", ondelete="restrict", index=True)
    check_in = fields.Datetime("Check In Date", required=True)
    check_out = fields.Datetime("Check Out Date", required=True)
    folio_id = fields.Many2one("tipd_hotel.folio", "Folio Number", ondelete="cascade")
    status = fields.Selection(related="folio_id.state", string="state")


class HotelFolio(models.Model):

    _name = "tipd_hotel.folio"
    ...
    tipd_hotel_invoice_id = fields.Many2one("account.move", "Invoice", copy=False)
    ...
    def _update_folio_line(self, folio_id):
        ...
        for rec in folio_id:
            for room_rec in rec.room_line_ids:
                room = tipd_hotel_room_obj.search(
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if "service_line_ids" not in vals and "folio_id" in vals:
                ...
                vals["order_policy"] = vals.get("tipd_hotel_policy", "manual")
                ...
                if not vals:
                    vals = {}
                vals["name"] = self.env["ir.sequence"].next_by_code("tipd_hotel.folio")
    ...
    def write(self, vals):
        ...
        for rec in self:
            ...
            if len(list(new_rooms)) != 0:
                ...
                for rm in room_list:
                    room_obj = tipd_hotel_room_obj.search([("product_id", "=", rm.id)])
            ...
            if not len(list(new_rooms)):
                ...
                for room in room_list_obj:
                    room_obj = tipd_hotel_room_obj.search([("product_id", "=", room.id)])
                    room_obj.write({"isroom": False})
                    room_vals = {
                        "room_id": room_obj.id,
                        "check_in": rec.checkin_date,
        ...
        for rec in self:
            ...
            for product in rec.room_line_ids.mapped("product_id"):
                if rec.room_line_ids.filtered(
                    ...
                    rooms = self.env["tipd_hotel.room"].search(
                        [("product_id", "=", product.id)]
                    )
...
class HotelFolioLine(models.Model):
    ...
    folio_id = fields.Many2one("tipd_hotel.folio", "Folio", ondelete="cascade")
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if "folio_id" in vals:
                folio = self.env["tipd_hotel.folio"].browse(vals["folio_id"])
    ...
    def _get_real_price_currency(self, product, rule_id, qty, uom, pricelist_id):
        ...
        product_uom_id = self.env.context.get("uom") or product.uom_id.id
        if uom and uom.id != product_uom_id:
    ...
    def _get_display_price(self, product):
        if hasattr(self.folio_id.pricelist_id, 'discount_policy') and self.folio_id.pricelist_id.discount_policy == "with_discount":
            return product.with_context(
                pricelist=self.folio_id.pricelist_id.id
            ).lst_price
            
        if self.folio_id.pricelist_id and hasattr(self.folio_id.pricelist_id, '_get_product_price'):
            return self.folio_id.pricelist_id._get_product_price(
                product, 
                self.product_uom_qty or 1.0, 
                uom=self.product_uom_id if self.product_uom_id else product.uom_id,
    ...
    def _onchange_product_id(self):
        ...
        if hasattr(product, 'sale_line_warn') and product.sale_line_warn != "no-message":
